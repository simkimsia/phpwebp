<?php
	...
	// Slide 5 aka Step 2
	$stmt = $mysqli->prepare("INSERT INTO `lesson11_users` (`userid`, `name`,`password`,`interests`) VALUES (?, ?, ?, ?)"); 


	/* SPECIAL NOTE: 
	* because $interests is an array due to the reason of using checkboxes
	* we need to implode it into a string
	*
	* see http://php.net/manual/en/function.implode.php
	* string implode ( string $glue , array $pieces )
	*/
	$interestsAsString = implode(",", $interests); 

	// Slide 6 aka Step 3 the bind params must correspond to the ?
	$stmt->bind_param("ssss", $userid, $name, $password, $interestsAsString); // 4 ???? so we use ssss. we use sss because userid, name, password, interests is varchar in the table 
